 ...
 *
 * A cluck object is used to object a cluster lock.
 *
 * This allows programs to run code in a cluster safe manner. A single
 * instance of that code can be running in an entire cluster of computers.
 *
 ...
 * the lock.
 *
 * The process can timeout or fail, in which case the on_error()
 * function gets called instead of the on_locked() one.
 */
